<template>
  <v-autocomplete :loading="loading" :items="items" :search-input.sync="search" :filter="()=>true" v-model="select"
    clearable :label="label" :disabled="disabled" @change="change">
    ...
    <template slot="item" slot-scope="{ item, tile }">
      <v-list-tile-content>
        ...
        <v-list-tile-sub-title v-for="(f,index) in fields" :key="index" v-if="index>0" v-text="item[f]"></v-list-tile-sub-title>
      </v-list-tile-content>
      <!-- <v-list-tile-action>
...
<script>
  ...
  export default {
    ...
    async created() {
      let {
        RelateFields
      } = await getModuleStrut(this.Relate)
    ...
    methods: {
      async querySelections(v) {
        ...
        let {
          Data
        } = await this.$http.post(`/api/${this.Relate}/list`, {
          Condition: {
            Filters: [{
