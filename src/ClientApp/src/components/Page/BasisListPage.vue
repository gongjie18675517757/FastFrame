      ...
      <v-flex xs12>
        <v-card>
          ...
          <v-card-title>
            ...
            <a-btn :moduleName="moduleInfo.name" name="Update" icon title="修改" :disabled="selection.length!=1 && !currentRow"
              @click="toEdit(currentRow || selection[0])">
              <v-icon>edit</v-icon>
            </a-btn>
            <a-btn @click="remove()" :disabled="!havSelection" icon title="删除" :moduleName="moduleInfo.name" name="Delete">
              <v-icon>delete</v-icon>
            </a-btn>
            <a-btn v-for="item in toolitems" :key="item.name" icon v-if="evalShow(item)" :title="item.title"
              :moduleName="moduleInfo.name" :name="item.name" :disabled="evalDisabled(item)" @click="evalAction(item)">
              ...
              <v-flex v-if="TreeKey" :xs3="TreeKey">
                <vue-perfect-scrollbar :class="[this.pageInfo.success?'dialog-page':'full-page']">
                  <v-treeview :active.sync="tree.active" :items="tree.items" :load-children="loadTreeItems" :open.sync="tree.open"
                    ...
                    transition @update:active="setCurrentTreeNode">
                    <v-icon v-if="!item.children.length" slot="prepend" slot-scope="{ item, active }" :color="active ? 'primary' : ''">mdi-account</v-icon>
                  </v-treeview>
                  ...
                  <v-data-table :headers="headers" :loading="loading" :items="items" :total-items="total"
                    ...
                    <template slot="items" slot-scope="props">
                      <tr :active="!singleSelection && props.selected" @click="handleRowClick(props)">
                        <td>
                          <v-icon small size="16" color="primary" v-if="singleSelection && currentRow==props.item">check</v-icon>
                          <v-checkbox v-if="!singleSelection" primary hide-details v-model="props.selected"></v-checkbox>
                        </td>
                        <!-- <td>
                  <v-avatar size="32">
                    <img
                    ...
                    >
                        ...
                        <td v-for="col in columns" :key="col.Name">
                          <Cell :info="col" :model="props.item" @toEdit="toEdit(props.item)" :moduleName="moduleInfo.name" />
                        </td>
                      </tr>
                    </template>
  ...
  export default {
    ...
    async created() {
      let {
        TreeKey,
        RelateFields
      } = await getModuleStrut(this.moduleInfo.name)
    ...
    methods: {
      ...
      async loadList() {
        ...
        try {
          let {
            Total,
            Data
          } = await this.$http.post('/api/' + this.moduleInfo.name + '/list', pageInfo)
          this.items = Data
          this.total = Total
        } finally {
          this.loading = false
      ...
      async loadTreeItems(item) {
        let {
          Data
        } = await this.$http.post(`/api/${this.moduleInfo.name}/List`, {
          PageIndex: 1,
          PageSize: 999,
          Condition: {
            Filters: [{
