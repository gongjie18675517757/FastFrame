    ...
    <v-card v-if="selectedNode" style="box-shadow: none;">
      ...
      <v-card-text :style="{ height: isDialog ? 'calc(100vh - 102px)' : null }">
        ...
        >
          ...
          <v-radio label="会签(多人时,全部人审核即通过)" value="and"></v-radio>
        </v-radio-group>
        <legend
        ...
        >
          ...
          <v-list dense>
            <v-list-item v-if="selectedNode.Checkers.length == 0">
              <v-list-item-title>
                未定义审核人
                <v-btn text small color="info" @click="addCheckerEnum = true"
                  >添加</v-btn
                >
              </v-list-item-title>
            </v-list-item>
            ...
            >
              ...
              <v-list-item-icon>
                <v-icon color="p" @click="removeChecker(rIndex)">close</v-icon>
              </v-list-item-icon>
            </v-list-item></v-list
          >
        </legend>
        ...
        >
          ...
          <v-list dense>
            <v-list-item v-if="selectedNode.Events.length == 0">
              <v-list-item-title>
                未定义事件
                <v-btn text small color="info" @click="addEventEnum = true"
                  >添加</v-btn
                >
              </v-list-item-title>
            </v-list-item>
