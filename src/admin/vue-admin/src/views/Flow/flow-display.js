class FlowDisplay {
    constructor(diagramDiv) {
        ...
        this.loadFlow = function (flowData) {
            ...
            if (pos)
                _displayer.initialPosition = go.Point.parse(pos);
            // 更改所有连线中间的文本背景色
            setLinkTextBg();
        };
        /**
        ...
        function init(divId) {
            ...
            window._displayer = _displayer;

            // 双击事件 
            _displayer.addDiagramListener("ObjectDoubleClicked", onObjectDoubleClicked);

            //单击事件
            _displayer.addDiagramListener("ObjectSingleClicked", onObjectDoubleClicked);
        }
        ...
        function makeNodeTemplate() {
            return G(go.Node, "Spot", { locationSpot: go.Spot.Center }, new go.Binding("location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify), { selectable: true, selectionAdornmentTemplate: makeNodeSelectionAdornmentTemplate() }, new go.Binding("angle").makeTwoWay(), G(go.Panel, "Auto", { name: "PANEL" }, new go.Binding("desiredSize", "size", go.Size.parse).makeTwoWay(go.Size.stringify), G(go.Shape, "RoundedRectangle", // default figure
                {
                    portId: "",
                    name: "PIPE",
                    fromLinkable: true,
                ...
                toolTip: G(go.Adornment, "Auto", G(go.Shape, { fill: "#FFFFCC" }), G(go.TextBlock, { margin: 4 }, new go.Binding("text", "", nodeInfo)))
            }));
        }
        /**
         * 选中节点的样式
        ...
        function makeNodeSelectionAdornmentTemplate() {
            return G(go.Adornment, "Auto", G(go.Shape, { fill: null, stroke: "deepskyblue", strokeWidth: 1.5, strokeDashArray: [4, 2] }), G(go.Placeholder));
        }
        /**
         * tooltip上显示的信息
         * @param d
        ...
        function makeLinkTemplate() {
            return G(go.Link, { selectable: false }, { relinkableFrom: true, relinkableTo: true, reshapable: true }, {
                routing: go.Link.AvoidsNodes,
                curve: go.Link.JumpOver,
                corner: 5,
                toShortLength: 4
            }, new go.Binding("layerName", "color"), new go.Binding("zOrder"), G(go.Shape, { isPanelMain: true, stroke: "black", strokeWidth: 3 }, new go.Binding("stroke"), new go.Binding("zOrder")), G(go.Shape, { isPanelMain: true, stroke: "gray", strokeWidth: 2 }), G(go.Shape, { isPanelMain: true, stroke: "white", strokeWidth: 1, name: "PIPE", strokeDashArray: [10, 10] }), G(go.Shape, { toArrow: "standard", stroke: null }, new go.Binding("stroke"), new go.Binding("fill"), new go.Binding("zOrder")), G(go.Panel, "Auto", G(go.Shape, {
                fill: null,
                stroke: null
