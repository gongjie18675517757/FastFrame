...
namespace FastFrame.Service.Services.Basis
{
    public partial class DeptService
    {
        protected override async Task OnGeting(DeptDto dto)
        {
            ...
            var (members, manages) = await EventBus.TriggerRequestAsync<(UserViewModel[] members, UserViewModel[] manages), string>(dto.Id);
            dto.Members = members;
        ...
        protected override async Task OnGetListing(IEnumerable<DeptDto> dtos)
        {
            ...
            var keyValuePairs = await EventBus.TriggerRequestAsync<IEnumerable<KeyValuePair<string, (UserViewModel[], UserViewModel[])>>, string[]>(keys);
            foreach (var item in dtos)
            {
                item.Managers = keyValuePairs.Where(v => v.Key == item.Id).SelectMany(v => v.Value.Item2);
                item.Members = keyValuePairs.Where(v => v.Key == item.Id).SelectMany(v => v.Value.Item1);
            }
        }
    }
}
